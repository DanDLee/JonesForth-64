\ -*- text -*-
\	A sometimes minimal FORTH compiler and tutorial for Linux / x86-64 systems. -*- asm -*-
\	By Richard W.M. Jones <rich@annexia.org> http://annexia.org/forth
\	64-bit conversion with updated comments by Claude
...
\ x86-64, the idivq instruction gives both anyway).  Now we can define the / and MOD in terms of /MOD
...
: PICK ( x_u ... x_1 x_0 u -- x_u ... x_1 x_0 x_u )
	...
	8 *		( multiply by the word size - 8 bytes on x86-64 )
...
: .S		( -- )
	...
	WHILE
		...
		SPACE
		8+		( move up - 8 bytes per cell on x86-64 )
	REPEAT
	DROP
;
...
: DEPTH		( -- n )
	S0 @ DSP@ -
	8 /			( divide by cell size to get count - 8 bytes on x86-64 )
...
: TO IMMEDIATE	( n -- )
	...
	8+		( increment to point at the value - 8 bytes per cell on x86-64 )
	STATE @ IF	( compiling? )
		' LIT ,		( compile LIT )
...
: ID.
	8+		( skip over the link pointer - 8 bytes on x86-64 )
...
: UNUSED	( -- n )
	...
	-
	8 /		( returns number of cells - 8 bytes per cell on x86-64 )
;
...
(
	...
	(4096 bytes or 512 cells on x86-64).
...
(
	...
	definition of the word.  Only a very tiny part of the x86-64 assembly space is covered, just
	enough to write a few assembler primitives below.
)

HEX

( Equivalent to the NEXT macro - 64-bit version with REX.W prefix )
...
( The x86-64 registers - encoding for registers 0-7 )
: RAX IMMEDIATE 0 ;
: RCX IMMEDIATE 1 ;
: RDX IMMEDIATE 2 ;
: RBX IMMEDIATE 3 ;
...
( x86-64 stack instructions - these work for RAX-RDI without REX prefix )
: PUSH IMMEDIATE 50 + C, ;
: POP IMMEDIATE 58 + C, ;
...
: RDTSC		( -- lsb msb )
	RDTSC		( writes the result in %rdx:%rax )
	RAX PUSH	( push lsb )
	RDX PUSH	( push msb )
;CODE
...
(
	...
	for DROP followed by DROP (eg. two 'pop %rax' instructions in this case).
...
HEX
( Check for 64-bit NEXT macro: 48 AD FF 20 )
: =NEXT		( addr -- next? )
	   DUP C@ 48 <> IF DROP FALSE EXIT THEN	( check REX.W prefix )
	1+ DUP C@ AD <> IF DROP FALSE EXIT THEN ( check LODSQ opcode )
